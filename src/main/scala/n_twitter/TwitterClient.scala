...
object TwitterClient {
  def getInstance : Twitter = {
    ...
    cb.setOAuthConsumerKey(TwitterConfiguration.apiKey)
    cb.setOAuthConsumerSecret(TwitterConfiguration.apiSecret)
    cb.setOAuth2AccessToken(TwitterConfiguration.accessToken)
    cb.setOAuthAccessTokenSecret(TwitterConfiguration.accessTokenSecret)

    val tf = new TwitterFactory(cb.build())
    tf.getInstance()
  }

  def receiveTweets(term: String) = {
...
class TwiterStreamClient(actorSystem: ActorSystem) {
